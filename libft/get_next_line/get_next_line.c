...
/*   Updated: 2022/07/18 13:55:57 by camillebarb      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_print_line(char *buffer_save)
{
	...
	while (buffer_save[i] && buffer_save[i] != '\n')
		i++;
	line = ft_substrr(buffer_save, 0, i + 1);
	if (!line)
	{
		free(line);
		return (NULL);
...
char	*ft_get_buffer_save(char *buffer_save)
{
	...
	while (buffer_save[i])
	{
		...
		{
			i += 1;
			new_buffer_save = ft_substrr(buffer_save, i, (strlen - i) + 1);
			break ;
		}
...
char	*ft_read(int fd, char *buffer_read)
{
	...
	while (ret > 0)
	{
		...
		buffer_read[ret] = '\0';
		if (buffer_save == NULL)
			buffer_save = ft_strdupp(buffer_read);
		else
			buffer_save = ft_strjoinn(buffer_save, buffer_read);
	}
